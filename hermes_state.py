...
class SessionDB:
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 7:
                # v7: add projects table and sessions.project_id column
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS projects (
                        name TEXT PRIMARY KEY,
                        path TEXT,
    ...
    def create_project(
        ...
        def _do(conn):
            conn.execute(
                """INSERT OR IGNORE INTO projects (name, path, description, config, created_at, updated_at)
                   VALUES (?, ?, ?, ?, ?, ?)""",
                (name, path, description, json.dumps(config) if config else None, now, now),
            )
        self._execute_write(_do)
        return name
